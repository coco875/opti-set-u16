...
pub struct SymmetricDifferenceScenario<T: SetInt> {
    bit_set: T,
    other: T,
}

impl<T: SetIntConstruct> ScenarioConstructor for SymmetricDifferenceScenario<T> {
...
impl<T: SetIntConstruct> Scenario for SymmetricDifferenceScenario<T> {
    fn task(&mut self) {
        self.bit_set.symmetric_difference_with(&self.other);
    }
}
