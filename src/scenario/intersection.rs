...
pub struct IntersectionScenario<T: SetInt> {
    bit_set: T,
    other: T,
}

impl<T: SetIntConstruct> ScenarioConstructor for IntersectionScenario<T> {
...
impl<T: SetIntConstruct> Scenario for IntersectionScenario<T> {
    fn task(&mut self) {
        self.bit_set.intersection_with(&self.other);
    }
}
