mod bit_tree_set;
mod byte_array_set;
mod custom_bit_set;
mod interval;
mod lib_avl_tree;
mod lib_bit_set;
mod lib_bitvec;
mod lib_fixed_bit_set;
mod lib_fx_hash_set;
mod lib_fx_hash_set_default_func;
mod lib_idlset;
mod lib_interval;
mod lib_range_set_blaze;
mod lib_rbtree;
mod lib_roaring;
mod lib_croaring;
mod simd_bit_set;
mod std_hash_set;
mod std_hash_set_default_func;
mod std_hash_set_no_hasher;
mod std_linked_list;
mod std_tree_set;
mod std_tree_set_default_func;
mod std_vec;
mod std_vec_dicotomie;

pub use bit_tree_set::BitTreeSet;
pub use byte_array_set::ByteArraySet;
pub use custom_bit_set::CustomBitSet;
pub use interval::IntervalSet;
pub use lib_avl_tree::LibAvlTree;
pub use lib_bit_set::LibBitSet;
pub use lib_bitvec::LibBitVec;
pub use lib_fixed_bit_set::LibFixedBitSet;
pub use lib_fx_hash_set::LibFxHashSet;
pub use lib_fx_hash_set_default_func::LibFxHashSetDefaultFunc;
pub use lib_idlset::LibIdlset;
pub use lib_interval::LibInterval;
pub use lib_range_set_blaze::LibRangeSetBlaze;
pub use lib_rbtree::LibRBTree;
pub use lib_roaring::LibRoaring;
pub use lib_croaring::LibCRoaring;
pub use simd_bit_set::SimdBitSet;
pub use std_hash_set::StdHashSet;
pub use std_hash_set_default_func::StdHashSetDefaultFunc;
pub use std_hash_set_no_hasher::StdHashSetNoHasher;
pub use std_linked_list::StdLinkedList;
pub use std_tree_set::StdTreeSet;
pub use std_tree_set_default_func::StdTreeSetDefaultFunc;
pub use std_vec::StdVec;
pub use std_vec_dicotomie::StdVecDicotomie;

pub trait SetInt: 'static {
    fn clear(&mut self);
    fn insert(&mut self, n: u16);
    fn remove(&mut self, n: u16) -> bool;
    fn contains(&self, n: u16) -> bool;
    fn len(&self) -> usize;
    fn iter(&self) -> Box<dyn Iterator<Item = u16> + '_>;
    fn union_with(&mut self, other: &Self);
    fn intersection_with(&mut self, other: &Self);
    fn difference_with(&mut self, other: &Self);
    fn symmetric_difference_with(&mut self, other: &Self);

    fn to_bytes(&self) -> [u8; 8192] {
        let mut bytes = [0; 8192];
        for n in self.iter() {
            bytes[(n / 8) as usize] |= 1 << (n % 8);
        }
        bytes
    }

    fn from_bytes(&mut self, bytes: &[u8]) {
        self.clear();
        for (i, &byte) in bytes.iter().enumerate() {
            if byte != 0 {
                for j in 0..8 {
                    if byte & (1 << j) != 0 {
                        self.insert((i * 8 + j) as u16);
                    }
                }
            }
        }
    }
}

pub trait SetIntConstruct: SetInt {
    fn new() -> Self;
    fn with_capacity(capacity: usize) -> Self;
}
